...
def fetch_openaq_raw_data_by_date(str_date, bucket_name, folder_name, zone, isSave):
    s3 = boto3.resource(
        service_name='s3',
        region_name=zone
    )
...
def process_raw_data(combined_single_day_data, str_date):
    data_by_country = combined_single_day_data[combined_single_day_data['country'] == "US"]
    unique_cities_usa = data_by_country['city'].unique()
    unique_parameter = combined_single_day_data['parameter'].unique()
    list_usa_city_data = []
    for city in unique_cities_usa:
        dict_city = {}
        print("CITY:", city)
        ...
        dict_city["parameters"] = list_parameter
        list_usa_city_data.append(dict_city)
    Path(datadir + "/country/usa/" + str_date).mkdir(parents=True, exist_ok=True)
    Path(datadir + "/country/usa/" + str_date + "/processed_data").mkdir(parents=True, exist_ok=True)
    filename = datadir + "/country/usa/" + str_date + "/processed_data/all_cities_usa.pkl"
    utilities.write_pickle_file(filename, list_usa_city_data)
    return
